    ...
    internal static class LoggerExtensions
    {
        ...
        public static void LogDelivery(
            ...
            => logger.LogInformation(
                $"The message delivered: producer={producer.Name}, " +
                $"key={key}, value={value}, offset={offset}.");

        public static void LogDeliveryError(
            this ILogger logger,
            IClient producer,
            string key,
            string value,
            string reason)
            => logger.LogError(
                $"Delivery failed: producer={producer.Name}, " +
                $"key={key}, value={value}, reason={reason}.");

        public static void LogConsume(
            this ILogger logger,
            IClient consumer,
            string key,
            string value,
            TopicPartitionOffset offset)
            => logger.LogInformation(
                $"The message received: consumer={consumer.Name}, " +
                $"key={key}, value={value}, offset={offset}.");
        ...
        public static void LogCommitError(
            this ILogger logger,
            IClient consumer,
            TopicPartitionOffset offset,
            Exception ex)
        ...
        public static void LogConsumeError(this ILogger logger, IClient consumer, Exception ex)
            => logger.LogError(
                ex,
                $"An error occured while executing the Consume method: " +
                $"consumer={consumer.Name}, message={ex.Message}.");
    }
}
